...
class OrderAccountIsolationTest(unittest.TestCase):
    ...
    def test_seller_order_sync_and_scheduler_exclude_self_buyer_orders(self):
        order_service = (ROOT / "common/services/order_service.py").read_text(encoding="utf-8")
        scheduler_source = _function_source(
            ROOT / "scheduler/app/services/scheduler/redelivery_task.py",
            "RedeliveryTask",
            "_get_pending_orders",
        )
        self.assertIn("买家身份与当前账号一致", order_service)
        self.assertIn("XYOrder.buyer_id != str(account.unb)", scheduler_source)
        self.assertIn("~self_purchase_exists", _function_source(
            ROOT / "common/services/order_service.py", "OrderService", "list_orders"
        ))
    ...
    def test_delivery_request_and_lookup_are_account_scoped(self):
        source = (ROOT / "websocket/app/api/routes/internal.py").read_text(encoding="utf-8")
    ...
    def test_order_page_actions_and_delivery_logs_are_account_scoped(self):
        ...
        self.assertIn("XYAutoReplyMessageLog.account_id", order_service)
        self.assertIn("XYAutoReplyMessageLog.order_no", order_service)
