...
class ProductMaterial(TimestampMixin, Base):
    ...
    shop_stock: Mapped[int | None] = mapped_column(BigInteger, comment="鱼小铺库存")
    shop_shipping_mode: Mapped[str | None] = mapped_column(String(20), comment="鱼小铺发货方式：free/distance/fixed/no_shipping")
    shop_shipping_fee: Mapped[float | None] = mapped_column(Numeric(8, 2), comment="鱼小铺一口价邮费")
    shop_support_pickup: Mapped[bool | None] = mapped_column(comment="鱼小铺是否支持自提")
    shop_fans_price_all: Mapped[float | None] = mapped_column(Numeric(12, 2), comment="鱼小铺全部粉丝价")
    shop_fans_price_old: Mapped[float | None] = mapped_column(Numeric(12, 2), comment="鱼小铺老粉价")
    shop_fans_price_bought: Mapped[float | None] = mapped_column(Numeric(12, 2), comment="鱼小铺已购粉价")
